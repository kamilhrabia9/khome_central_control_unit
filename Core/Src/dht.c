...
//###############################################################################################################
bool  DHT_readData(DHT_t *dht, float *Temperature, float *Humidity,uint8_t *array_data,int index)
{
  uint32_t  startTime;
  switch(dht->type)
  {
    ...
    case DHT_Type_AM2305:
      ...
      while(1)
      {
        ...
        if(HAL_GetTick() - dht->time > 1)
        {
          ...
          array_data[index+2] = (data[0]*256+ data[1])/10; // Relative humidity integral
          array_data[index] =  (data[2]*256+ data[3])/10;// Celsius integral
          array_data[index+1] =  (data[2]*256+ data[3])%((int)((data[3])/10)); // Celsius decimal
